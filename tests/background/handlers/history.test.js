...
describe('handleSaveToHistory', () => {
    ...
    it('should save to history successfully', async () => {
        ...
        expect(mockSls).toHaveBeenCalledWith({ summaryHistory: [{ videoId: 'vid1', title: 'Title', summary: 'Sum', timestamp: 1234567890 }] });
    ...
    it('should handle existing history', async () => {
        ...
        mockSlg.mockResolvedValue({ summaryHistory: [{ videoId: 'vid1', title: 'Old', summary: 'Old', timestamp: 1000 }] });
        mockSls.mockResolvedValue();
    ...
    it('should limit history to 100', async () => {
        ...
        const existing = Array(100).fill().map((_, i) => ({ videoId: `vid${i}`, title: 'T', summary: 'S', timestamp: i }));
