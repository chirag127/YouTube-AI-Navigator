...
describe('handleTranscribeAudio', () => {
    ...
    it('should transcribe audio successfully', async () => {
        ...
        mockFt.mockResolvedValueOnce({ ok: true, arrayBuffer: vi.fn().mockResolvedValue(new ArrayBuffer(10)) });
        const mockClient = { generateContent: vi.fn().mockResolvedValue('[{"start": 0, "text": "Hello"}]') };
        ...
        expect(mockRsp).toHaveBeenCalledWith({ success: true, segments: [{ start: 0, text: 'Hello' }] });
    });
    ...
    it('should handle JSON parse failure and extract array', async () => {
        ...
        mockSg.mockResolvedValue({ apiKey: 'key' });
        mockFt.mockResolvedValueOnce({ ok: true, arrayBuffer: vi.fn().mockResolvedValue(new ArrayBuffer(10)) });
        const mockClient = { generateContent: vi.fn().mockResolvedValue('Some text [{"start": 1}] more') };
        mockGeminiClient.mockReturnValue(mockClient);
        mockJp.mockImplementationOnce(() => { throw new Error('Parse error'); });
