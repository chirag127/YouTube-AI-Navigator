...
describe('handleFetchTranscript', () => {
    ...
    describe('handleFetchTranscript', () => {
        it('should fetch transcript successfully with json3', async () => {
            ...
            expect(mockFt).toHaveBeenCalledWith('https://www.youtube.com/api/timedtext?v=vid1&lang=en&fmt=json3');
            expect(mockRsp).toHaveBeenCalledWith({ success: true, data: [{ start: 1, duration: 2, text: 'Hello' }] });
        });
        ...
        it('should return error if all methods fail', async () => {
            ...
            expect(mockRsp).toHaveBeenCalledWith({ success: false, error: 'All transcript fetch methods failed' });
        });
    });
});