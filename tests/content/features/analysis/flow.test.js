...
describe('startAnalysis', () => {
    ...
    it('should analyze successfully', async () => {
        ...
        msg.mockResolvedValueOnce({ externalApis: { deArrow: { enabled: true, usePrivateAPI: true } } });
        const metadataExtractor = (vi.mocked(await import('../../../extension/content/metadata/extractor.js'))).metadataExtractor;
        metadataExtractor.extract.mockResolvedValue('metadata');
        const extractTranscript = (vi.mocked(await import('../../../extension/content/transcript/strategy-manager.js'))).extractTranscript;
        extractTranscript.mockResolvedValue({ success: true, data: 'transcript' });
        const getComments = (vi.mocked(await import('../../../extension/content/handlers/comments.js'))).getComments;
        getComments.mockResolvedValue('comments');
        const analyzeVideo = (vi.mocked(await import('../../../extension/content/features/analysis/service.js'))).analyzeVideo;
        analyzeVideo.mockResolvedValue({ success: true, data: { segments: 'segments', summary: 'summary' } });
    ...
    it('should handle analysis failure', async () => {
        ...
        const showError = (vi.mocked(await import('../../../extension/content/ui/components/loading.js'))).showError;
