...
vi.mock('../../../extension/content/core/state.js', () => ({
    state: { currentVideoId: '123', settings: { autoLike: true, autoLikeThreshold: 50, autoLikeLive: true, likeIfNotSubscribed: true } },
}));
...
describe('AutoLiker', () => {
    ...
    describe('isLiveStream', () => {
        it('should detect live stream', () => {
            const qs = (vi.mocked(await import('../../../extension/utils/shortcuts/dom.js'))).qs;
            qs.mockReturnValue({ style: { display: 'block' } });
            Object.defineProperty(window, 'getComputedStyle', { value: vi.fn(() => ({ display: 'block' })) });
    ...
    describe('clickLikeButton', () => {
        it('should click like button', async () => {
            ...
            qsa.mockReturnValue([{ closest: vi.fn(() => true), click: vi.fn(), getAttribute: vi.fn(() => 'false') }]);
