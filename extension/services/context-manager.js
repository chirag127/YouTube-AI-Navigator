...
export class ContextManager {
    constructor(settings) {
        ...
        this.apis = {
            ...
            igdb: new IgdbAPI(
                ...
                this.settings.twitchAccessToken
            ),
            ...
            factcheck: new GoogleFactCheckAPI(
                this.settings.googleFactCheckApiKey
            ),
    ...
    async fetchContext(metadata) {
        ...
        if (
            ...
            addTask(
                "musicbrainz_artist",
                this.apis.musicbrainz.searchArtist(author)
            );
            addTask(
                "musicbrainz_release",
                this.apis.musicbrainz.searchRelease(title, author)
            );
        }
        ...
        if (category === "Science & Technology" || category === "Education") {
            addTask(
                "semanticscholar",
                this.apis.semanticscholar.searchPaper(title)
            );
        ...
        console.log(
            `[ContextManager] Processing ${results.length} API call results...`
        );
        results.forEach((result, index) => {
            if (result.status === "fulfilled") {
                ...
                if (res && (Array.isArray(res) ? res.length > 0 : true)) {
                    ...
                    console.log(
                        `[ContextManager] - ${name}: no valid data returned`
                    );
                }
            } else {
                ...
                if (err.message?.includes("timeout")) {
                    console.warn(
                        `[ContextManager] ${name} timeout - API may be slow or unavailable`
                    );
                } else if (
                    ...
                    console.warn(
                        `[ContextManager] ${name} network error - check connectivity`
                    );
                } else if (
                    ...
                    console.warn(
                        `[ContextManager] ${name} auth error - check API keys`
                    );
                } else {
                    console.warn(
                        `[ContextManager] ${name} unexpected error - investigate further`
                    );
                }
            }
        });
