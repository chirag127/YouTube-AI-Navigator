...
async function fetchViaGenius(videoId, lang = 'en') {
    ...
    if (!title) { w('[Genius Strategy] Could not find video title'); return null; }
    l(`[Genius Strategy] Checking if music video: "${title}" by ${channel}`);
    const isMusic = await isMusicVideo(title, channel || '');
    if (!isMusic) { l('[Genius Strategy] Gemini determined this is NOT a music video. Skipping.'); return null; }
    ...
    const result = await new Promise(resolve => {
        rt.sendMessage({ type: 'GET_LYRICS', title, artist: channel || '' }, response => {
            if (rt.lastError) { w('[Genius Strategy] Message error:', rt.lastError); resolve(null); return; }
