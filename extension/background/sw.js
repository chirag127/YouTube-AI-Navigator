import { l, w, e, rt, tab, url } from "../utils/shortcuts.js";

rt.onInstalled.addListener(async (details) => {
    if (details.reason === "install") {
        l("YouTube AI Master installed");
        try {
            const onboardingUrl = url("onboarding/onboarding.html");
            await tab({ url: onboardingUrl });
        } catch (x) {
...
rt.onMessage.addListener((request, sender, sendResponse) => {
    ...
    (async () => {
        try {
            ...
            const { validateMessage, sanitizeRequest } = await import(
            ...
            switch (action) {
                case "TEST":
                    ...
                    break;
                case "GET_SETTINGS": {
                    const { handleGetSettings } = await import(
                ...
                case "FETCH_TRANSCRIPT": {
                    const { handleFetchTranscript } = await import(
                        "./handlers/fetch-transcript.js"
                    );
                    await handleFetchTranscript(sanitized, sendResponse);
                    break;
                }
                case "ANALYZE_VIDEO": {
                    const { handleAnalyzeVideo } = await import(
                        "./handlers/analyze-video.js"
                    );
                    await handleAnalyzeVideo(sanitized, sendResponse);
                    break;
                }
                case "ANALYZE_COMMENTS": {
                    const { handleAnalyzeComments } = await import(
                ...
                case "CHAT_WITH_VIDEO": {
                    const { handleChatWithVideo } = await import(
                        "./handlers/chat.js"
                    );
                ...
                case "SAVE_TO_HISTORY": {
                    const { handleSaveToHistory } = await import(
                        "./handlers/history.js"
                    );
                ...
                case "SAVE_HISTORY": {
                    const { handleSaveHistory } = await import(
                        "./handlers/video-data.js"
                    );
                    sendResponse(await handleSaveHistory(sanitized));
                    break;
                }
                case "GET_METADATA": {
                    const { handleGetMetadata } = await import(
                        "./handlers/metadata.js"
                    );
                    await handleGetMetadata(sanitized, sendResponse);
                    break;
                }
                case "FETCH_INVIDIOUS_TRANSCRIPT": {
                    const { handleFetchInvidiousTranscript } = await import(
                        "./handlers/invidious.js"
                    );
                    sendResponse(
                ...
                case "FETCH_INVIDIOUS_METADATA": {
                    const { handleFetchInvidiousMetadata } = await import(
                        "./handlers/invidious.js"
                    );
                ...
                case "GET_CACHED_DATA": {
                    const { handleGetCachedData } = await import(
                        "./handlers/cache.js"
                    );
                ...
                case "SAVE_CHAT_MESSAGE": {
                    const { handleSaveChatMessage } = await import(
                        "./handlers/chat-history.js"
                    );
                    await handleSaveChatMessage(sanitized, sendResponse);
                    break;
                }
                case "SAVE_COMMENTS": {
                    const { handleSaveComments } = await import(
                        "./handlers/comments-storage.js"
                    );
                    await handleSaveComments(sanitized, sendResponse);
                    break;
                }
                case "TRANSCRIBE_AUDIO": {
                    const { handleTranscribeAudio } = await import(
                        "./handlers/transcribe-audio.js"
                    );
                    await handleTranscribeAudio(sanitized, sendResponse);
                    break;
                }
                case "GET_LYRICS": {
                    const { handleGetLyrics } = await import(
                        "./handlers/get-lyrics.js"
                    );
                    await handleGetLyrics(sanitized, sendResponse);
                    break;
                }
                case "GET_VIDEO_DATA": {
                    const { handleGetVideoData } = await import(
                        "./handlers/video-data.js"
                    );
            ...
            e("Background handler error:", error);
