...
async function handleChat() {
  ...
  try { const a = await gs.chatWithVideo(q, ctx); await updateMsg(lid, a) }
  catch (e) { await updateMsg(lid, `Error: ${e.message}`) }
}
...
async function updateMsg(id, t) { const d = document.getElementById(id); if (d) { d.innerHTML = await parseMarkdown(t); ch.scrollTop = ch.scrollHeight } }
  ...
  try {
    ...
    if (tr.error) {
      if (tr.error.includes('does not have captions')) {
        showError('No Captions Available', 'This video does not have captions/subtitles. Please try a different video that has closed captions enabled.')
      } else {
    ...
    // Generate analysis
    const opts = await chrome.storage.local.get(['summaryLength', 'targetLanguage'])
    const so = { length: opts.summaryLength || 'Medium', language: opts.targetLanguage || 'English' }
    ...
    const cr = await chrome.tabs.sendMessage(tab.id, { action: 'GET_COMMENTS' }).catch(() => ({ comments: [] }))
...
function showLoadingState() {
  sc.innerHTML = '<div class="empty-state"><svg viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z"/></svg><p class="empty-state-title">Generating summary...</p></div>'
  ic.innerHTML = '<div class="empty-state"><svg viewBox="0 0 24 24"><path d="M9 21c0 .55.45 1 1 1h4c.55 0 1-.45 1-1v-1H9v1zm3-19C8.14 2 5 5.14 5 9c0 2.38 1.19 4.47 3 5.74V17c0 .55.45 1 1 1h6c.55 0 1-.45 1-1v-2.26c1.81-1.27 3-3.36 3-5.74 0-3.86-3.14-7-7-7z"/></svg><p class="empty-state-title">Generating insights...</p></div>'
  tc.innerHTML = '<div class="empty-state"><svg viewBox="0 0 24 24"><path d="M14 17H4v2h10v-2zm6-8H4v2h16V9zM4 15h16v-2H4v2zM4 5v2h16V5H4z"/></svg><p class="empty-state-title">Loading transcript...</p></div>'
}
...
async function renderMd(t, el) { el.innerHTML = await parseMarkdown(t) }
