...
import { msg, sg, ce, st, l, w } from '../../utils/shortcuts.js';
...
export async function renderComments(c) {
  ...
  try {
    ...
    if (r.success) {
      ...
      c.innerHTML = `<div class="yt-ai-markdown"><h3>💬 Comment Sentiment Analysis</h3>${html}<hr><h4>Top Comments (${cm.length})</h4>${cm
        .slice(0, 5)
        .map(
          x =>
