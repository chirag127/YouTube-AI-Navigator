export const buildContextString = ({
  ...
  let title = `Original Title: ${metadata?.originalTitle || metadata?.title || 'Unknown'}`;
  if (metadata?.deArrowTitle) title += `\nCommunity Title (DeArrow): ${metadata.deArrowTitle}`;
      ...
      sponsorBlockSegments
        .map(seg => {
          ...
          return `- [${seg.category}] ${formatTime(
            seg.start
          )} - ${formatTime(seg.end)}${desc} (${seg.votes} votes${seg.locked ? ', locked' : ''})`;
        })
        .join('\n');
  }
