...
#### **`services/dearrow/`** (DeArrow Integration)
-   **`api.js`**: Fetches crowdsourced titles and thumbnails from the DeArrow API to replace clickbait.

#### **`services/api/`** (External APIs)
-   **`invidious.js`**: Client for the Invidious API (alternative YouTube frontend). Used for fetching transcripts/metadata when direct methods fail.
-   **`piped.js`**: Client for the Piped API (another alternative frontend). Used as a secondary fallback.
